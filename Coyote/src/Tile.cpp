...
namespace coyote
{
        ...
        out_ports_l2_reqs_(num_l2_banks_)
    {
        ...
        coyote::CacheDataMappingPolicy b_pol=coyote::CacheDataMappingPolicy::PAGE_TO_BANK;
        if(bank_policy_=="page_to_bank")
        {
            b_pol=coyote::CacheDataMappingPolicy::PAGE_TO_BANK;
        }
        else if(bank_policy_=="set_interleaving")
        {
            b_pol=coyote::CacheDataMappingPolicy::SET_INTERLEAVING;
        }
        else
        {
        ...
        coyote::VRegMappingPolicy s_pol=coyote::VRegMappingPolicy::CORE_TO_BANK;
        if(scratchpad_policy_=="core_to_bank")
        {
            s_pol=coyote::VRegMappingPolicy::CORE_TO_BANK;
        }
        else if(scratchpad_policy_=="vreg_interleaving")
        {
            s_pol=coyote::VRegMappingPolicy::VREG_INTERLEAVING;
        }
        ...
        {
            coyote::CacheDataMappingPolicy t_pol=coyote::CacheDataMappingPolicy::PAGE_TO_BANK;
            if(tile_policy_=="page_to_bank")
            {
                t_pol=coyote::CacheDataMappingPolicy::PAGE_TO_BANK;
            }
            else if(tile_policy_=="set_interleaving")
            {
                t_pol=coyote::CacheDataMappingPolicy::SET_INTERLEAVING;
            }
            else
            {
                printf("Unsupported cache data mapping policy\n");
	...
	std::shared_ptr<ArbiterMessage> msg = std::make_shared<ArbiterMessage>();
        ...
        msg->is_core = true;
        msg->id = req->getCoreId();
        msg->type = coyote::MessageType::CACHE_REQUEST;
    ...
    void Tile::handle(std::shared_ptr<coyote::CacheRequest> r)
    {
        access_director->putAccess(r);
    }
    
    void Tile::handle(std::shared_ptr<coyote::ScratchpadRequest> r)
    {
        access_director->putAccess(r);
    }

    void Tile::handle(std::shared_ptr<coyote::MCPUSetVVL> r)
    {
        if(!r->isServiced())
        {
    ...
    void Tile::handle(std::shared_ptr<coyote::MCPUInstruction> r)
    {
        //TODO: The actual MCPU that will handle the request needs to be defined
	std::shared_ptr<NoCMessage> mes = std::make_shared<NoCMessage>(r, NoCMessageType::MCPU_REQUEST, 32, id_, corresponding_mcpu);
	std::shared_ptr<ArbiterMessage> msg = std::make_shared<ArbiterMessage>();
    ...
    void Tile::handle(std::shared_ptr<coyote::InsnLatencyEvent> r)
    {
    ...
    void Tile::insnLatencyCallback(const std::shared_ptr<coyote::InsnLatencyEvent>& r)
    {
        r->setServiced();
        request_manager_->notifyAck(r);
    }
