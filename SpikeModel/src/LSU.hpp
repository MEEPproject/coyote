...
{
    class VariableLatencyDispatcher;
    ...
    public:
        ...
        public:
            ...
            PARAMETER(bool, tlb_always_hit, true, "L1 TLB will always hit")
        ...
        public:
            ...
            MemoryAccessInfo(MemoryInstruction inst_ref, memory_access_t a) :
                ldst_inst_ref_(inst_ref),
            ...
            MemoryInstruction getInst() const { return ldst_inst_ref_; }
            ...
            bool operator ==(const MemoryAccessInfo & m) const
            {
                return m.getInst().getBits()==getInst().getBits();
            }

        private:

            // load/store instruction pointer
            MemoryInstruction ldst_inst_ref_;
        ...
        */
        ...
        class MemoryAccessInfoHash
        {
            public:
                size_t operator()(const MemoryAccessInfo& m) const
                {
                     return m.getInst().getBits();
                }
        };
        ...
        public:
            ...
            // and we will use this pointer to query values from functions of ExampleInst class
            const MemoryInstruction getInst() const {
                return mem_access_info_ptr_->getInst();
            }
        ...
        */
        ...
        {
            std::cout << "Setting the TLB\n";
            tlb_cache_ = &tlb;
        }

        // Issue/Re-issue ready instructions in the issue queue
        bool issueInst_(MemoryInstruction & insn, bool isReIssue=false);

        void setDispatch(spike_model::VariableLatencyDispatcher * d)
        {
            dispatcher_=d;
        }

    private:
        spike_model::VariableLatencyDispatcher * dispatcher_;
        ...
        unsigned latency_last_instruction;

        unsigned latency_cache_miss_=10;
        unsigned latency_tlb_miss_=25;
        ...
        void getInstsFromDispatch_(std::shared_ptr<MemoryInstruction>);

        // Receive MSS access acknowledge from Bus Interface Unit
        void getAckFromBIU_(const MemoryAccessInfoPtr & mem_access_info_ptr);


        // Handle MMU access request
        bool handleMMULookupReq_(const MemoryAccessInfoPtr & mem_access_info_ptr);
        ...
        void popIssueQueue_(std::shared_ptr<MemoryInstruction>);
        ...
        void rehandleMMULookupReq_(const MemoryAccessInfoPtr & mem_access_info_ptr);
        ...
        void rehandleCacheLookupReq_(const MemoryAccessInfoPtr & mem_access_info_ptr);

        // Reload cache line
        void reloadCache_(uint64_t);

        // Update issue priority after dispatch
        void updateIssuePriorityAfterNewDispatch_(std::shared_ptr<MemoryInstruction>);

        // Update issue priority after TLB reload
        void updateIssuePriorityAfterTLBReload_(std::shared_ptr<MemoryInstruction>,
                                                const bool = false);

        // Update issue priority after cache reload
        void updateIssuePriorityAfterCacheReload_(std::shared_ptr<MemoryInstruction>,
                                                  const bool = false);

        // Update issue priority after store instruction retires
        void updateIssuePriorityAfterStoreInstRetire_(std::shared_ptr<MemoryInstruction>);
        ...
        std::unordered_multiset<MemoryAccessInfo, MemoryAccessInfoHash> pending_insts_mmu;
        std::unordered_multiset<MemoryAccessInfo, MemoryAccessInfoHash> pending_insts_cache;
        sparta::Counter count_cache_accesses_=sparta::Counter(getStatisticSet(), "count_cache_accesses", "Number of cache accesses generated by the lsu instructions", sparta::Counter::COUNT_NORMAL);
        sparta::Counter count_cache_misses_=sparta::Counter(getStatisticSet(), "count_cache_misses", "Number of cache misses", sparta::Counter::COUNT_NORMAL);
    };

    inline std::ostream & operator<<(std::ostream & os,
        const spike_model::LSU::MemoryAccessInfo::MMUState & mmu_access_state){
    ...
    inline std::ostream & operator<<(std::ostream & os,
        const spike_model::LSU::MemoryAccessInfo::CacheState & cache_access_state){
    ...
    inline std::ostream& operator<<(std::ostream& os,
        const spike_model::LSU::LoadStoreInstInfo::IssuePriority& rank){
        switch(rank){
    ...
    inline std::ostream& operator<<(std::ostream& os,
        const spike_model::LSU::LoadStoreInstInfo::IssueState& state){
