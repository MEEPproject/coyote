...
auto spike_model::CPUFactory::bindTree_(sparta::RootTreeNode* root_node,
    ...
    if(topology_->topology_name=="tiled")
    {
        ...
        {
            noc=root_node->getChild(std::string("arch.noc"))->getResourceAs<spike_model::NoC>();

            //CacheBank * bank=root_node->getChild(std::string("arch.tile0.l2_bank0"))->getResourceAs<spike_model::CacheBank>();
