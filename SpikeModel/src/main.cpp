...
std::shared_ptr<SimulationOrchestrator> createTraceDrivenOrchestrator(sparta::app::CommandLineSimulator& cls, std::shared_ptr<SpikeModel>& sim, spike_model::DetailedNoC* detailed_noc)
{
    ...
    spike_model::SimulationEntryPoint * entry_point=NULL;
    ...
    {
        printf("Well, well...\n");
        entry_point=sim->getRoot()->getChild(std::string("arch.l2_bank"))->getResourceAs<spike_model::SimulationEntryPoint>();
    }
    else if(architecture=="memory_controller_unit_test")
    {
        entry_point=sim->getRoot()->getChild(std::string("arch.memory_controller"))->getResourceAs<spike_model::SimulationEntryPoint>();
    }
    else
    {
