...
namespace spike_model
{
    ...
    {
        str << "0x" << std::hex << req.getAddress() << " @ " << req.getTimestamp() << ", type: " << (int)req.getCommand() << ", serviced: " << (int)req.isServiced() << ", destCoreID: " << req.getCoreId() << ", destRegID: " << req.getDestinationRegId() << ", size: " << req.getSize();
        return str;
    }
}
#endif
